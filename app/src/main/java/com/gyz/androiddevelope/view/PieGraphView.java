...
import com.gyz.androiddevelope.util.GeomTool;
...
public class PieGraphView extends View {
    ...
    private Paint mPaintOuter, mPaintInner, mTextPaint, mItemCenterPaint, mCenterCirclePaint;
    private String totalAssets, totalMoneyStr;
    ...
    private boolean needAnimation = true, needTouchMode = true, needCenterPoint = true;
    /**
    ...
    private void initPaints() {
        ...
        mCenterCirclePaint.setColor(getResources().getColor(R.color.color_eeeeee));
    }
...
//        画总负债或总资产
    ...
    private void drawText(Canvas canvas) {
        ...
        mTextPaint.setColor(getResources().getColor(R.color.color_999999));
        Paint.FontMetricsInt fontMetricsInt = mTextPaint.getFontMetricsInt();
        float strY;
//        精确计算文字位置
        strY = (centerY - 35) + (fontMetricsInt.bottom - fontMetricsInt.top) / 2 - fontMetricsInt.bottom;

        canvas.drawText(totalAssets, centerX, strY, mTextPaint);
...
//        下部分数字
        ...
        Paint.FontMetricsInt fontMetrics = mTextPaint.getFontMetricsInt();
        float numY;
        numY = (centerY + 40) + (fontMetrics.bottom - fontMetrics.top) / 2 - fontMetrics.bottom;

        canvas.drawText(totalMoneyStr, centerX, numY, mTextPaint);
    }
    ...
    private void drawGrownPie(Canvas canvas) {
        ...
        //画内圆分隔线
        canvas.drawCircle(centerX, centerY, smallCircleRadius - 20, mCenterCirclePaint);
    }
    ...
    private void updateBounds(int w, int h) {
        ...
        smallCircleRadius = bigOvalRadius - mRingWidth + bigOvalPadding;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (needTouchMode && event.getAction() == MotionEvent.ACTION_DOWN && mAnimMode == ANIM_MODE_NONE) {
    ...
    public void setNeedTouchMode(boolean needTouchMode) {
        this.needTouchMode = needTouchMode;
    }

    public void setNeedCenterPoint(boolean needCenterPoint) {
        this.needCenterPoint = needCenterPoint;
    }
